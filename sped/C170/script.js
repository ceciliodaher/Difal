...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let sharedPeriodo = "";
    // Variáveis para análise de registros 0200 e C170
    let catalogoProdutos = {};
    let dadosResultado = [];
    let contadorEncontrados = 0;
    let contadorNaoEncontrados = 0;
    let todasColunas = [];
    ...
    async function processarSpedParaExcel(fileContent, caminhoSaidaExcel) {
        ...
        // const { nomeEmpresa, periodo } = extrairInformacoesHeader(registros); // Already done in processSpedFile
         ...
         // Verificar se temos registros necessários para análise C170+NCM
        const temC170 = registros['C170'] && registros['C170'].length > 0;
        const tem0200 = registros['0200'] && registros['0200'].length > 0;

        if (temC170 && tem0200) {
            updateStatus('Criando catálogo de produtos...', 30);
            addLog('Criando catálogo de produtos dos registros 0200...', 'info');
            await criarCatalogoProdutos(registros);

            updateStatus('Processando registros C170...', 40);
            addLog('Processando registros C170 e vinculando NCM...', 'info');
            await processarC170(registros);

            addLog(`Análise C170+NCM concluída: ${contadorEncontrados} encontrados, ${contadorNaoEncontrados} não encontrados`, 'success');
        }
     ...
     */
    /**
     * Cria catálogo de produtos a partir dos registros 0200 - VERSÃO CORRIGIDA
     */
    async function criarCatalogoProdutos(registros) {
        addLog('=== INICIANDO CRIAÇÃO DO CATÁLOGO DE PRODUTOS ===', 'info');
        catalogoProdutos = {};
        const registros0200 = registros['0200'] || [];
        addLog(`Processando ${registros0200.length} registros 0200`, 'info');
        ...
        for (let i = 0; i < registros0200.length; i++) {
            ...
            try {
                ...
                if (i < 3) {
                    console.log(`Debug 0200[${i}]:`, linha0200);
                    console.log(`Comprimento: ${linha0200.length}`);
                }

                // CORREÇÃO: Não usar filter() - trabalhar com índices diretos
                if (linha0200.length >= 9) {
                    ...
                    if (i < 5) {
                        console.log(`0200[${i}] - COD: '${codigoItem}', NCM: '${ncm}', DESC: '${descricao.substring(0, 20)}...'`);
                    }

                    // Só adicionar se o código do item não estiver vazio
                    if (codigoItem && codigoItem.trim()) {
                        catalogoProdutos[codigoItem] = {
        ...
        addLog(`Catálogo criado: ${Object.keys(catalogoProdutos).length} produtos`, 'success');
    }

    /**
     * Processa registros C170 e vincula com NCM do catálogo - VERSÃO CORRIGIDA
     */
    async function processarC170(registros) {
        addLog('=== PROCESSANDO REGISTROS C170 ===', 'info');
        dadosResultado = [];
        contadorEncontrados = 0;
        contadorNaoEncontrados = 0;

        // Definir colunas exatamente como no Python
        const colunasC170 = [
        ...
        const colunas0200 = ['NCM_PRODUTO', 'DESCR_CADASTRAL', 'TIPO_ITEM', 'STATUS_VINCULACAO'];
        todasColunas = colunasC170.concat(colunas0200);

        const registrosC170 = registros['C170'] || [];
        addLog(`Processando ${registrosC170.length} registros C170`, 'info');

        for (let i = 0; i < registrosC170.length; i++) {
            const linhaC170 = registrosC170[i];
            try {
                // Debug: Mostrar estrutura das primeiras linhas
                if (i < 3) {
                    console.log(`Debug C170[${i}]:`, linhaC170);
                }

                if (linhaC170.length >= 4) {
                    // CORREÇÃO: Não usar filter() - trabalhar com índices diretos
                    // Layout C170: ['', 'REG', 'NUM_ITEM', 'COD_ITEM', ...]
                    const codigoItemC170 = linhaC170[3] || ""; // COD_ITEM no C170 (índice 3)

                    // Debug dos códigos
                    if (i < 5) {
                        console.log(`C170[${i}] - COD: '${codigoItemC170}'`);
                    }

                    // Montar linha de resultado preservando a estrutura original
                    const linhaResultado = [];

                    // Preencher campos C170 mantendo índices corretos
                    for (let j = 0; j < colunasC170.length; j++) {
                        const indiceOriginal = j + 1; // j+1 porque índice 0 é vazio
                        const valor = linhaC170[indiceOriginal];

                        if (valor !== undefined && valor !== null && valor.trim() !== '') {
                            linhaResultado.push(valor.trim());
                        } else {
                            linhaResultado.push("SEM DADOS NA ORIGEM");
                        }
                    }

                    // Buscar no catálogo
                    if (catalogoProdutos[codigoItemC170]) {
                        const produto = catalogoProdutos[codigoItemC170];
                        linhaResultado.push(
                            produto.ncm,
                            produto.descricao,
                            produto.tipo,
                        ...
                        contadorNaoEncontrados++;
                    }

                    dadosResultado.push(linhaResultado);
                }
            } catch (error) {
                console.warn(`Erro linha ${i} do C170:`, error);
                continue;
            }
        ...
        addLog(`C170 processado: ${dadosResultado.length} registros totais`, 'success');
        addLog(`Encontrados: ${contadorEncontrados}, Não encontrados: ${contadorNaoEncontrados}`, 'info');
    }
    
    /**
     * Cria aba específica para análise C170+NCM - VERSÃO CORRIGIDA
     */
    async function criarAbaAnaliseC170NCM(context) {
        const { writer, logger } = context;
        try {
            const sheetAnalise = writer.addSheet('Analise_C170_NCM'); // Removido caracteres especiais

            // Estilos simplificados para evitar corrupção
            const headerStyle = {
                bold: true,
                fill: "D7E4BC", // Cor mais simples
                border: true
            };
            ...
            // Escrever cabeçalhos
            todasColunas.forEach((header, idx) => {
                const cell = sheetAnalise.cell(1, idx + 1);
                cell.value(header).style(headerStyle);

                // Definir largura das colunas de forma mais conservadora
                let width = 15;
                if (header === 'DESCR_COMPL' || header === 'DESCR_CADASTRAL') {
                    width = 40;
                } else if (header === 'COD_ITEM') {
                    width = 20;
                } else if (header.includes('VL_')) {
                    width = 18;
                }

                sheetAnalise.column(idx + 1).width(width);
            });

            // Escrever dados com tratamento de erro robusto
            for (let rowIdx = 0; rowIdx < dadosResultado.length; rowIdx++) {
                const linha = dadosResultado[rowIdx];

                for (let colIdx = 0; colIdx < linha.length; colIdx++) {
                    try {
                        const valor = linha[colIdx];
                        const cell = sheetAnalise.cell(rowIdx + 2, colIdx + 1);

                        // Tratamento de valores monetários
                        if (todasColunas[colIdx] && todasColunas[colIdx].includes('VL_')) {
                            const numeroValor = parseFloat(valor);
                            if (!isNaN(numeroValor)) {
                                cell.value(numeroValor).style(numStyle);
                            } else {
                                cell.value(valor || "").style(cellStyle);
                            }
                        } else if (todasColunas[colIdx] === 'STATUS_VINCULACAO') {
                            // Formatação condicional simplificada
                            if (valor === 'ENCONTRADO') {
                                cell.value(valor).style({
                                    ...cellStyle,
                                    fill: "C6EFCE"
                                });
                            } else {
                        ...
                        console.warn(`Erro ao processar célula [${rowIdx + 2}, ${colIdx + 1}]:`, cellError);
                        // Fallback: valor simples sem formatação
                        sheetAnalise.cell(rowIdx + 2, colIdx + 1).value(linha[colIdx] || "");
                    }
                }

                // Yield periodicamente para evitar travamento
                if (rowIdx % 1000 === 0) {
                    await new Promise(resolve => setTimeout(resolve, 10));
                }
            }

            logger.info(`Aba Analise_C170_NCM criada com ${dadosResultado.length} registros`);

        } catch (error) {
            console.error('Erro ao criar aba de análise C170+NCM:', error);
            logger.error(`Erro ao criar aba de análise: ${error.message}`);
            throw error;
        }
    }
    
    /**
     * Cria aba com estatísticas da análise
     */
    async function criarAbaEstatisticas(context) {
        const { writer, logger } = context;
        try {
            const sheetStats = writer.addSheet('Estatísticas');
            ...
            const dataStyle = { border: true, horizontalAlignment: "center" };

            // Título
            sheetStats.cell(1, 1).value('ANÁLISE C170 + NCM - ESTATÍSTICAS').style(titleStyle);
            sheetStats.range("A1:B1").merged(true);

            // Cabeçalhos
            sheetStats.cell(3, 1).value('Métrica').style(headerStyle);
            sheetStats.cell(3, 2).value('Valor').style(headerStyle);

            // Dados
            const total = dadosResultado.length;
            const percentual = total > 0 ? ((contadorEncontrados / total) * 100).toFixed(2) : 0;

            const estatisticas = [
                ['Total de Registros C170', total.toLocaleString()],
                ['NCMs Encontrados', contadorEncontrados.toLocaleString()],
                ['NCMs Não Encontrados', contadorNaoEncontrados.toLocaleString()],
                ['Taxa de Sucesso', `${percentual}%`],
                ['Produtos no Catálogo', Object.keys(catalogoProdutos).length.toLocaleString()]
            ];

            estatisticas.forEach((item, idx) => {
                sheetStats.cell(4 + idx, 1).value(item[0]).style(dataStyle);
                sheetStats.cell(4 + idx, 2).value(item[1]).style(dataStyle);
            });

            // Ajustar larguras
            sheetStats.column(1).width(25);
            sheetStats.column(2).width(20);

            addLog('Aba de estatísticas criada com sucesso', 'success');

        } catch (error) {
            console.error('Erro ao criar aba de estatísticas:', error);
            addLog(`Erro ao criar aba de estatísticas: ${error.message}`, 'error');
        }
    }
    ...
    async function gerarExcel(registros, nomeEmpresa, periodo, caminhoSaida) {
        ...
        try {
            ...
            // ADICIONAR ANÁLISE C170+NCM
            if (dadosResultado && dadosResultado.length > 0) {
                updateStatus('Criando análise C170+NCM...', 87);
                addLog('Criando aba de análise C170+NCM...', 'info');
                await criarAbaAnaliseC170NCM(context);
            }
